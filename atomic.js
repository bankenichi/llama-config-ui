...
function updateAtomicApplicability(config = null) {
  ...
  const notes = {
    ternary: 'DSpark uses the matching sidecar, lossless q8_0/q8_0 draft KV, four draft tokens by default, at most 16 draft slots, and a runtime-enforced 4096 positions per parallel slot. Media turns fall back safely and later text drafting rebuilds coherently.',
    qwen: 'NextN is embedded in the target GGUF and uses synchronous multi-step drafting. Leave the draft path empty. Target KV is turbo4/turbo3; the small draft context is explicitly f16/f16.',
    gemma26: 'The standard profile is target-only. Select the MTP preset to load the external assistant head. With vision configured, text drafts while real media turns use the safe target path.',
